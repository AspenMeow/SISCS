...
--and p_pers_data_effdt.emplid = p_names.emplid	
--and p_pers_data_effdt.emplid = p_citizenship.emplid	
--and p_pers_data_effdt.emplid = p_person.emplid	
--and p_citizenship.country = 'USA'	
--and p_citizenship.citizenship_status = s_citizen_sts_tbl.citizenship_status	
--and s_citizen_sts_tbl.country = p_citizenship.country	
and (p_names.effdt = (	
select	
max(b_ed.effdt)	
...
and p_pers_data_effdt.emplid ='161640972'
...
and t.effdt <= sysdate
left  join (select * from siscs.t_xlattable_vw_av where edw_actv_ind='Y' and edw_curr_ind='Y')  t_xlattable_vw 
on t_xlattable_vw.fieldname = 'ETHNIC_GROUP'	
and t_xlattable_vw.fieldvalue = t.ethnic_group	
and t_xlattable_vw.eff_status = 'A'	) re
where n=1
and emplid in ('161852124','104214101','107421384')
)
...
from dat
where HISP_LATINO='N' and setid='USA'
group by emplid ) b
